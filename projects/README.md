...
| [thync_as_migration_design.md](thync_as_migration_design.md) | thynC AS이력(수기 엑셀 원장 3,537행·174병원) → 기기현황 마이그 — X열 완료 판정·오입력 자동 보정 3종·88건 충돌 절충·병원별 단계 적용(초기必 검증 게이트). 메디인 리허설 완료(dev2) | **완료(PROD 2026-09-05) — 잔여 보류 병원**(replay 후 `backfill-device-condition.mts --dry` 재실행으로 상태축 정리, `device_condition_location_design.md` §9.4) |

...
| [hospital_device_registry_design.md](hospital_device_registry_design.md) | 병원별 웨어러블 디바이스 원장 — `/devices` 시리얼 개체(전역 1행) + append 이벤트 이력(등록·병동이동·회수·교체 쌍), 병원별 병동 마스터, 검토형 임포트, 딜 계약 수량 대조, 기존 `hospital_devices` 수량표 폐기(D1~D12, P0~P5). 2.0 기획 A1 구체화 | **설계 확정 — 구현 중(P1~P4, feat/device-registry, 2026-09-01 착수)** |

...
- **검증**: 미처리(fulfilled_at NULL)·종결 전 상태 / 요청 라인 전부 포함 / 품목 해석 실패 → '선택한 인벤토리에 재고 품목 없음' / **전량 일치**(시리얼 수·LOT 합 = 요청 수량) / 시리얼·재고·LOT 검증은 기존 엔진(`planInventoryTransaction`) 위임 / **기기현황 사전 검증**: 심전계·산소포화도본체 시리얼이 타 병원 ACTIVE면 라인 오류(진행 불가), 같은 병원 ACTIVE는 등록 skip 경고 · **기기 상태 축(2026-09-17 — `device_condition_location_design.md` §9.3)**: 출고 확정 `registerDevicesIn(source WMS)`은 암묵으로 기기 상태 IN_USE·위치 병원을 기록(공용 암묵 전이 헬퍼 — 이 모듈 코드 변경 없음). **폐기(`condition='SCRAPPED'`) 시리얼은 사전 검증에서 라인 단위 오류로 올려야 하나 미구현** — 현재는 tx 안의 `registerDevicesIn` 409 '폐기된 기기입니다 — 정정 후 등록하세요'로 출고 확정 전체가 실패(막히긴 하나 `?preview=true` 라인 판정에 드러나지 않음). 후속: `findUnitsBySerial` 결과의 `condition==='SCRAPPED'`를 라인 err로 1분기 추가

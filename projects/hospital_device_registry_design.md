...
3. **모델별 compare**: ECG = Σ(models ECG 행)+Σ(fallback 디바이스수), 항상 hard. SpO2 = **SpO2 행이 하나라도 있으면 실측 hard**(Σ SpO2 행 — 폴백 딜은 기여 없음), 폴백 전용 병원만 구 soft(ECG 동수 가정 = Σ디바이스수). 링BP = BP 행이 있을 때만 hard(Σ BP 행), 없으면 `none`. `deals=0`이면 전부 null/`none`. product_type별(`byType`)도 같은 규칙.
4. 딜별 `expectedByModel {ecg,spo2,bp}`(행 없는 모델은 null; 폴백 딜은 `{ecg: 디바이스수}` + ⓘ '모델별 수량 미입력') — 계약별 표(§6.1)·요약 `deals[]`가 사용. 커버리지 `dl` CTE도 ECG 기대를 같은 models-first 산식으로 계산(`expectedByType` 툴팁 포함).
`intro_beds`는 표시만. 딜 저장 시 파생 없음(매번 조인). `getExpectedDeviceCount`(Σ디바이스수)는 구 축 호환용으로 유지(`expectedDeviceCount`·근거 딜 팝오버). **평가용 제외(2026-09-01, B-21)**: `models[].active`는 배치 중 전체, `activeEval`은 그중 `usage_type=EVAL`, `activeForCompare = active − activeEval`, **`diff = activeForCompare − expected`**(hard). 병원 `evalTotal`, 커버리지 행 `activeEcg`(EVAL 제외)·`activeEcgEval`·`evalTotal`·`diff = activeEcg − expected`, 전역 합계 `active.eval`. 미지정(NULL)은 판매용과 같이 대조에 포함.
...
| B-25 | **계약 수량 = 딜 모델별 수량(`sales_deal_devices`) 1순위 · 디바이스수 폴백** — 2026-09-02 제품 책임자 결정("계약 수량은 모델별 — 심전도 100 + 산소포화도 50"). 산식·compare 개편은 §9.1, 계약별 표 모델 열은 §6.1. PROD 기준 249딜 중 ~57딜만 모델 행 보유 — 나머지는 폴백(ⓘ 표기)로 점진 백필. SpO2는 실측 행이 있으면 'ECG 동수 가정' soft를 벗고 hard 대조. **링 혈압계는 `device_info.quantity_tracked` 토글로 딜 폼 '기기별 도입 수량'에 노출 가능**(현재 false — 운영 결정 사안, BP 행이 생기면 hard 대조 자동 활성). sales_* 테이블 쓰기 금지 |

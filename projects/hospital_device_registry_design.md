...
- **[병원별]**(`?view=hospital&hospital=&tab=list|history|wards|import&status=&model=&ward=&q=&page=&device=`)
  ...
  - 병원 미선택(첫 화면): **축약 병원 커버리지 표**(`GlobalCoverage compact` — 사용자 피드백 2차 "첫 페이지에 아무것도 안 보이는 건 별로, 병원별 기기현황 리스트가 보여야"). 컬럼 `병원 | 상태 | 배치 중 | 계약 | 차이 | 회수(30일) | 마지막 이벤트 | →`만 — '배치 중'은 병원 요약 한 줄과 같은 수(전 모델 합계 + 평가용 n 병기), 계약·차이는 ECG 대조(평가용 제외). 툴바 = 필터 [전체|미등록만|차이 있음|등록 완료] + 검색 [병원명/코드](정렬은 '차이 큰 순' 고정, 셀렉트 숨김), 50행 페이지, 행 클릭 → 그 병원 선택(setHospital), 미등록 행만 [임포트] 버튼. 숨김: SpO2/GW/평가용/마지막 임포트 열·'상품유형 혼합' 배지·전역 요약 줄. 전체 12열 모드·전역 최근 이벤트 탭은 v1 UI에 없음(파일·API 보존). `MobileActionBar`도 렌더하지 않음. URL: 병원 미선택 시 `q`/`page`는 이 표의 병원명 검색·페이지(병원 선택 시 초기화).

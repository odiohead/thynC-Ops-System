...
> **상태: 완료 (PROD 배포 2026-09-02, 커밋 d6e54b1)** — P1~P4 + 3층 구조·용도·상품유형·계약건·AS표시·UI v1. PROD 원장 데이터는 사용자 직접 입력. 후속(P6): 교체 허용량 규칙·WMS 훅·유지보수 기기 조치 패널·온프렘 diff·AI 도구 — 2026-09-01 쟁점 A-1~A-8 추천안 확정 후 같은 날 사용자 착수 지시("dev2에서 형상을 보고 수정보완"), 브랜치 `feat/device-registry`. P0: PROD 09-01 01:00 덤프로 dev2 동기화(hospital_devices 132·딜 249·units 21,085)
...
> **명칭: 기기 현황**(2026-09-02 개명 — nav·페이지 제목·Excel 파일명 `기기현황_*`·사용자 노출 문구는 '기기 현황', 문서 내 '원장' 명칭은 개념 용어로 유지)
...
- **[병원별]**(`?view=hospital&hospital=&tab=list|history|wards|import&status=&model=&ward=&q=&page=&device=`)
  ...
  - 2행 **계약건(딜)별 현황 표**(B-23·B-25, 2026-09-02 — 구 '요약 한 줄' 대체, `HospitalContractTable`): 열 `계약건 | 유형 | 심전계(도입/등록) | 산소포화도(도입/등록) | 혈압계(도입/등록) | 교체 건수`. 모델 셀 = 1행 '도입 n / 등록 m' + 2행(muted) 'AS a · 누적 b'(2026-09-02 3차 개정 — a=AS진행중 플래그 수 `asByModel`, b=**누적 AS건수 = 교체 짝 수** `replacementsByModel`(회수 유닛 모델 축), 둘 다 0이면 2행 숨김, 툴팁에 4값). 도입은 딜 모델별 수량(그 딜에 그 모델 수량이 없으면 '—'; 수량 미입력 딜은 전 모델 '—' + 심전계 셀 ⓘ '모델별 도입 기기 수량 미입력 — 딜 상세(규모·계약 카드)에서 입력' — 디바이스수 폴백 제거 2026-09-02), 등록은 그 계약건×모델의 배치 중 수(`activeByModel`). 마지막 '교체 건수' 열은 **행 합계로 유지**(축 밖 모델(GW 등) 교체 포함 — 모델 셀 누적 합보다 클 수 있어 중복 아님). 우측 병원 단위 'AS진행중 n' 칩 유지. 행 = 계약완료 딜 ∪ 배치·교체에 등장한 코드(계약 외 코드는 '(계약 외)' 표시 — 재적재로 끊긴 코드 발견 경로), 딜 없는 배치·교체가 있으면 '(미지정)' 행(도입 '—' + 모델별 등록), 마지막 합계 행(모델별 도입 Σ/등록 Σ)의 ⓘ가 구 팝오버(모델별 hard/soft 대조·근거 딜·평가용 제외·상품유형별·교체 집계)를 연다. 모바일(sm 미만)은 딜당 압축 줄 `E 100/98 · S 50/50 · BP —/0 · 교체 n`. 헤더 줄에 'AS진행중 n' warning 칩(B-24)·'상품유형 혼합' 배지·배치 중/회수(30일)/병동 요약. **개체 상태 표시 라벨(B-24)**: 목록·드로어·export의 상태는 `사용중`(ACTIVE)/`AS진행중`(ACTIVE+플래그, warning)/`회수됨` — '배치 중'은 집계 수치 문구에만. **용어(2026-09-02 3차 개정): 액션은 'AS 접수'**(구 'AS 표시'), 상태 라벨 'AS진행중'·[AS 해제]·코드 AS_OPEN/AS_CLEAR는 불변. **기기 목록 필터(compact, 2026-09-02 정리)**: 상태 세그먼트 + 시리얼 검색 + **상품유형·계약건 인라인 select**만 상시 노출([필터 더보기] 제거 — 모델 칩·병동·WMS·용도·'AS진행중만'·정렬·행수는 전체(비-compact) 모드 전용; 병동 값은 URL `ward=` 경유로만 동작, 필터 초기화가 함께 리셋). [열 더보기]에 계약건 열, 행 ⋯과 **드로어 버튼 줄([병동 이동][회수][교체] 옆 — 2026-09-02 PROD 갭 보완)**에 **'AS 접수'**(유지보수 코드 선택 소형 모달)/'AS 해제', 선택 바에 [계약건 지정](SET_DEAL)·**[AS 접수]**(AsFlagModal 다건 모드 — MNT 코드·업무일자 공유, bulk AS_OPEN)·**[AS 해제]**(선택에 AS진행중 행이 있을 때만, bulk AS_CLEAR). 매트릭스 표·WMS 열·평가용 칩은 노출하지 않음(`SummaryStrip.tsx`는 보존, 미사용).
  - 3행 소형 탭 **기기 목록 | 이력 | 병동 | 임포트**(기본 기기 목록) — `EventsTab`/`WardPanel`/`ImportPanel`은 그대로. 우측에 [선택] 토글(USER+, 기기 목록 탭) + [Excel](기기 목록/이력 탭 기준).
  - 기기 목록 = `DeviceTable compact showSelection={선택 토글}`: 기본 열 `시리얼 | 모델 | 병동 | 상태 | 상품유형 | 배치일 | 최근 이벤트 | ⋯`, 용도·회수일·사유·연결·창고 개체·메모·원문 2행은 **[열 더보기]**; 필터는 상태(배치 중/회수됨/전체) + 시리얼 검색만 기본, 모델 칩·병동·WMS·용도·상품유형·정렬·행수는 **[필터 더보기]**(숨은 필터에 값이 있으면 자동 펼침). 체크박스·선택 바(`BulkActionBar`)는 [선택] 토글을 켤 때만(기본 off). 행 ⋯ 메뉴(이동/회수/교체/정정)·빈 상태 문구는 동일. **2026-09-02 개정(행 밀도·병동 탭)**: compact 기본 열은 모델 셀 1줄(기기명만, 모델코드 툴팁)·시리얼 원문 2줄 제거(툴팁 '원문 …', ⚠ 인라인 유지)·셀 패딩 py-2.5→py-1.5(헤더 포함, [열 더보기] 시 2줄 복귀·모바일 카드 무변경); 상태 필터 줄 아래 **병동 탭 칩 바** `[전체 n][6병동 n]…[미지정 n]`(카운트 = summary.wards ACTIVE 수 — 추가 요청 없음, 클릭 = 기존 `ward=` 필터라 [필터 더보기] 병동 셀렉트와 자동 동기화, 활성 탭 강조·재클릭 해제, overflow-x 가로 스크롤; 폐쇄 병동은 배치>0일 때만 '(폐쇄)' 표기, 미지정 칩은 n>0일 때만).
...
**AS진행중 플래그(B-24, 2026-09-02)**: `openDeviceAs(ctx,{deviceId})` → AS_OPEN(이미 표시 409 `이미 AS진행중으로 표시된 기기입니다`·회수됨/타 병원 409, ctx.ref(MAINTENANCE)가 `as_ref_code`) / `clearDeviceAs(ctx,{deviceId})` → AS_CLEAR(표시 없음 409 `AS진행중 표시가 없는 기기입니다`). 교체·회수는 fold가 자동 해제(이벤트 없음). `cancelLastEvent` LIFO 취소 지원(AS_CLEAR 취소 → 플래그 복원, AS_OPEN 취소 → 해제). 라우트 `POST /api/devices/units/[id]/as-open`·`/as-clear`(write, audit `hospital_device` UPDATE **'AS 접수'/'AS 해제'** — 2026-09-02 용어 개정: 액션 'AS 접수', 이벤트 라벨 `DEVICE_EVENT_TYPE_LABELS.AS_OPEN='AS 접수'`; 코드·DB 어휘 불변). 목록 필터 `?as=1`·`?deal=<code|none>`. **일괄(2026-09-02 후속)**: `bulkDeviceAction({ action:'AS_OPEN'|'AS_CLEAR', deviceIds })` — 같은 병원 ACTIVE만(기존 bulk 규칙), 기기마다 AS 이벤트를 **한 트랜잭션·같은 action_group·같은 ref(ctx.ref MNT)·같은 업무일자**로 기록, 이미 표시/미표시는 `skipped[]`(전부면 409 `선택한 기기가 모두 AS진행중입니다`/`선택한 기기에 AS진행중 표시가 없습니다`) — audit `hospital_device_event` 'AS 일괄 접수'/'AS 일괄 해제'.
...
## 9. 기존 데이터·모듈 연동
### 9.1 딜 기대 수량 — **모델별 수량 단일 소스(B-25, 2026-09-02 개정 — 당일 재개정: 디바이스수 폴백 제거)**
...
2. **폴백 없음(2026-09-02 제품 책임자 지시)**: 모델 행이 없는 딜은 `expectedByModel=null`·`expectedSource='none'` — **대조에서 제외**. `daewoong_device_count`는 어떤 도입/대조 수치에도 쓰지 않는다(참고 표기 전용 — 계약 팝오버 '대웅 디바이스수 합 n (참고)').
3. **모델별 compare**: 각 모델(ECG·SpO2·링BP) = **그 모델 행이 병원 계약완료 딜에 하나라도 있으면 hard(Σ 행)**, 하나도 없으면 `none`(expected null·diff null) — ECG 포함. 'ECG 동수 가정' SpO2 soft는 **제거**(타입 어휘만 호환 유지, 서버는 soft를 생산하지 않음). product_type별(`byType`)도 같은 규칙(모델 행 합).
4. 딜별 `expectedByModel {ecg,spo2,bp}`(행 없는 모델은 null; 수량 미입력 딜은 전체 null + ⓘ '모델별 도입 기기 수량 미입력 — 딜 상세(규모·계약 카드)에서 입력') — 계약별 표(§6.1)·요약 `deals[]`가 사용. 커버리지 `dl` CTE도 같은 산식(`sum(m.ecg)` — 전 딜 미입력이면 expected NULL): '차이 있음' 필터·'차이 큰 순' 정렬은 expected 보유 병원만(NULL은 뒤·미매치), 판매유형 배지는 수량 무관 딜 유형(`deal_normal/deal_lite`).
`intro_beds`는 표시만. 딜 저장 시 파생 없음(매번 조인). `getExpectedDeviceCount`(Σ디바이스수)는 **참고 데이터로만 유지**(`expectedDeviceCount`·근거 딜 팝오버 '(참고 — 대조 미사용)' 라벨). **평가용 제외(2026-09-01, B-21)**: `models[].active`는 배치 중 전체, `activeEval`은 그중 `usage_type=EVAL`, `activeForCompare = active − activeEval`, **`diff = activeForCompare − expected`**(hard). 병원 `evalTotal`, 커버리지 행 `activeEcg`(EVAL 제외)·`activeEcgEval`·`evalTotal`·`diff = activeEcg − expected`, 전역 합계 `active.eval`. 미지정(NULL)은 판매용과 같이 대조에 포함.
**상품유형별 기대 수량(2026-09-01, B-22)**: 같은 조인을 `GROUP BY sd.product_type`으로 — `SELECT sd.product_type, count(*), SUM(COALESCE(sd.daewoong_device_count,0)) FROM sales_deals sd JOIN status_codes sc ON sc.id=sd.status_id WHERE sd.hospital_code=$1 AND sc.category='SALES_DEAL_STATUS' AND sc.name='계약완료' GROUP BY 1`(`getHospitalProductTypeContext.byType` — 참고 수치). 요약 `models[].byProductType[type|'미지정'] = { active, activeForCompare, expected(그 유형 딜 모델 행 합 — hard 축만·compare none이면 null), diff(hard만) }`(키 = 계약 딜 유형 ∪ 배치 유형, '미지정'은 배치가 있을 때만; 모델 합계는 기존 필드 그대로), 병원 `productTypes[]`(ECG 기준 축)·`productTypeMixed`(딜 2종 또는 배치에 상품유형 존재 → UI 매트릭스)·`productTypeContext`·`replacements{ total, byType, last30d }`(`countReplacements(code,{from,to})` = 같은 병원 RECOVER 가운데 REGISTER와 교체 짝(`related_device_id` 또는 같은 action_group)이 있는 것, RECOVER 스냅샷 상품유형 기준 — 이관 쌍 제외). 커버리지 행 `productTypeMixed`(딜 2종)·`unassignedProductType`(혼합 병원의 NULL ACTIVE 수), 전역 `mixedProductTypeHospitals`.
**계약건(딜)별 집계(2026-09-02, B-23)**: 요약 `deals[] = { dealCode, roundNo|null, productType|null, contractDate|null, expected|null(Σ daewoong_device_count — 참고 표기 전용·계약 외 코드는 null), contracted, active(배치 중), replacements }` — 키는 계약완료 딜 ∪ ACTIVE 배치·교체 이벤트에 등장한 deal_code(재적재로 끊긴 코드도 '(계약 외)'로 노출). `dealUnassigned = { active(deal_code NULL ACTIVE 수), replacements }` · `asInProgress`(as_started_on NOT NULL ACTIVE 수 — B-24). 교체는 `countReplacementsByDeal` — `countReplacements`와 같은 RECOVER↔REGISTER 짝 판정을 RECOVER 이벤트 `deal_code` 스냅샷으로 그룹. `getExpectedDeviceCount.contractedDeals[]`에 `productType` 추가(additive).
...
| B-25 | **계약 수량 = 딜 모델별 수량(`sales_deal_devices`) 단일 소스 — 디바이스수 폴백 없음** — 2026-09-02 제품 책임자 결정("계약 수량은 모델별") + 당일 재지시("`daewoong_device_count`는 절대 쓰지 말 것 — 유일 소스는 딜 상세 '규모·계약' 카드의 '도입 기기 수량'"). 산식·compare 개편은 §9.1, 계약별 표 모델 열은 §6.1. 수량 미입력 딜은 도입 '—' ⓘ(대조 제외 — dev2 기준 계약완료 딜 보유 211병원 중 158병원이 전 딜 미입력 → 도입 '—', 점진 백필). SpO2·ECG 모두 실측 행이 있을 때만 hard('ECG 동수 가정' soft 제거). **링 혈압계는 `device_info.quantity_tracked` 토글로 딜 폼 '기기별 도입 수량'에 노출 가능**(현재 false — 운영 결정 사안, BP 행이 생기면 hard 대조 자동 활성). sales_* 테이블 쓰기 금지 |

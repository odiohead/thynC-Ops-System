...
async function tick() {
  ...
  try {
    ...
    if (r.registered || r.linked || r.failed || r.completedBack || r.shipBack || r.pickupBack || r.intakeBack) {
      console.log(`[channeltalk-as] 틱 완료 (scanned=${r.scanned}, registered=${r.registered}, linked=${r.linked}, waiting=${r.waiting}, failed=${r.failed}, completedBack=${r.completedBack}, shipBack=${r.shipBack}, pickupBack=${r.pickupBack}, intakeBack=${r.intakeBack})`)
    }
  } catch (err) {

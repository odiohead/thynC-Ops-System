...
- **AS진행중 플래그(2026-09-02 B-24)**: ACTIVE 배치의 표시 플래그(`as_started_on`·`as_ref_code`, 제3의 상태 아님) — 행 ⋯ [AS 표시](유지보수 코드 선택 소형 모달 → AS_OPEN 이벤트)/[AS 해제](AS_CLEAR), **교체·회수 시 자동 해제**(이벤트 없음), 이력에 'AS 시작/해제' 행. 개체 상태 표시는 `사용중`/`AS진행중`(warning)/`회수됨`('배치 중'은 집계 문구 전용), 목록 'AS진행중만' 체크(`?as=1`), 병원 요약 'AS진행중 n' 칩(`asInProgress`), Excel 상태 열 갱신 + 'AS 시작일' 열
...
| POST | `/api/devices/units/[id]/as-open` · `/as-clear` | 'AS진행중' 표시 / 해제 (write, B-24 — ref는 유지보수 코드 권장(`as_ref_code`), 이미 표시/표시 없음·회수됨 409, 교체·회수 시 자동 해제, audit 'AS 시작/해제') |

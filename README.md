...
- **전표 메타 수정 (2026-07-19 · 권한 강화 2026-07-20 — ADMIN이면서 재고 담당자 풀 등록자만)**: `PUT /api/inventory/transactions/[id]` — 유형(같은 시스템 동작 부류 내에서만: 일반↔일반·회수↔회수·폐기↔폐기)·요청자·**입출고일**·출고처·병원/업무 연결·비고. **수량·품목·위치·시리얼은 수정 불가**(재고 스냅샷·개체 정합 — 취소 후 재등록). 취소·이관(구) 전표 수정 불가, 감사 로그 before/after 기록

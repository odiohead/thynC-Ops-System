...
│   ├── devices/                      # 디바이스 원장 전역 — can-manage·summary(+export)·units(+[id] GET/PATCH·move·recover·as-open·as-clear·**repair-done·repair-undo·scrap·location**(기기 상태·위치 축 드로어 액션 4종 + `_unitState.ts` 라인 동기화·감사 공용, 2026-09-17)·bulk)·lookup·maintenance-lookup·events(+[id] PATCH/DELETE·export)·export
...
│   ├── as-receipts/                  # AS접수 CRUD + match(시리얼 원장 매칭 미리보기) + [id]/resolve-items(라인 결과 확정 — 기기현황 연동, 라인별 처리내용) + [id]/ship-info(기기군 발송정보 갱신, 2026-09-11) + export(라인 단위 Excel — 발송일 필터, 2026-09-07) + [id]/repair-done·scrap-line(라인 수리완료 체크/해제·라인 기기 폐기 — 기기 상태·위치 축, 2026-09-17) (등록 시 AS 티켓 자동 생성·AS 표시, 2026-09-04)
...
├── deviceRegistry/                   # 디바이스 원장 서비스 계층(유일한 쓰기자 — core·wms·write·**condition**(기기 상태·위치 축 2026-09-17 — intake/repair/scrap/moveLocation·암묵 전이 헬퍼·취소 규약)·import·admin·read·hooks, index 재수출; `lib/deviceRegistry` 임포트). 백필: `scripts/backfill-device-condition.mts`
├── deviceRegistryShared.ts           # 디바이스 원장 클라이언트 안전 상수 — 상태/이벤트 라벨·전이표·normalizeSerial·parseSerialLines·normalizeWardName·detectOnpremHeader·refLink · 기기 상태 6종/거점 DEVICE_SITE/스냅샷 이벤트 상수(2026-09-17)
...
├── asReceiptSearch.ts                # AS접수 목록·상세 공용 서버 조회 — 검색어 OR(운송장 정규화 일치)·중복접수 대조 (2026-09-18)
...
- **HospitalDevice (`hospital_devices`, 이름 승계 — 배치 프로젝션)**: 유닛당 0..1행(`device_id` UNIQUE FK → device_units) — `status`(ACTIVE/RECOVERED, 이벤트 fold 파생값)·`hospital_code`/`ward_id`(ACTIVE 위치, 복합 FK DEFERRABLE)·`last_hospital_code`·`placed_on`·`recovered_on`·`recover_reason_id`·`last_event_type/on`·`replaced_by_id`(→ 교체기 유닛)·`ext_device_code`(온프렘 닉네임 — 병원 문맥 값이라 배치에 둠)·`ext_last_seen_at`/`ext_synced_at`. 식별 컬럼(serial·model·mac·memo)은 갖지 않는다. 구 병원×모델 수량표는 마이그 내 `hospital_devices_qty_backup_202609`로 보존 후 DROP · `product_type`(TEXT CHECK 일반/라이트, NULL=미지정 — **상품유형은 자리의 판매 조건: 배치 속성**(B-22, 2026-09-01), REGISTER 이벤트 fold·CORRECT 갱신, 교체 시 신 배치가 상속, 회수 시 마지막 값 보존 후 재등록이 재지정; 인덱스 (hospital_code, product_type, status)) · `deal_code`(TEXT, **계약건(딜) 소프트 참조**(B-23, 2026-09-02) — `sales_deals.deal_code`, FK 없음(재적재 대비), REGISTER fold·CORRECT 갱신·교체 상속·회수 시 마지막 값 보존, 부분 인덱스 (hospital_code, deal_code)) · `as_started_on`/`as_ref_code`(**AS진행중 플래그**(B-24) — ACTIVE 배치의 표시 플래그, AS_OPEN set / AS_CLEAR·RECOVER·재REGISTER clear, ref는 유지보수 코드)
- **HospitalDeviceEvent (`hospital_device_events`)**: append-first 이력 — `device_id`(→ **device_units**)·`event_type`(REGISTER/MOVE_WARD/RECOVER/CORRECT/AS_OPEN/AS_CLEAR — AS 2종은 비상태 표시 이벤트(B-24): 플래그만 fold, last_event 제외 · **+ INTAKE 입고/REPAIR_DONE 수리 완료/SCRAP 폐기/SITE_MOVE 위치 이동(2026-09-17 — 10종, 배치 fold 비상태 이벤트 B-35 · 회수 기기라 `hospital_code` NULL 허용 B-31(알 수 있으면 배치/마지막 병원) · `changes` NOT NULL 강제)**)·`occurred_on`(업무일자)·`from_ward_id`/`to_ward_id`·`reason_code_id`·`related_device_id`(교체·이관 상대 → device_units)·`action_group`(교체·이관·일괄·임포트 묶음)·`ref_type/ref_code`(MAINTENANCE 등 연결)·`source`(MANUAL/IMPORT/WMS/ONPREM)·`import_batch_id`·`actor_*`·`edited_*`·`changes`(CORRECT · **스냅샷 이벤트(REGISTER·RECOVER·AS_OPEN·AS_CLEAR·INTAKE·REPAIR_DONE·SCRAP·SITE_MOVE·상태 CORRECT — MOVE_WARD 제외)에 `{ condition:{before,after}, location:{before,after,note?} }` 상태·위치 스냅샷(B-28 — 값이 같아도 기록, `location` 값 `{kind:'HOSPITAL'|'SITE'|null, code}`, `note`는 교체 회수 시 '입고 미확인' A-4). I-6: 유닛 값 = 그 유닛 id 최대 스냅샷 이벤트의 after — 취소는 id 순 판정(이후 스냅샷 있으면 409)·`changes.before` 복원·배치 축 취소 후 재도출(B-32)**) · `product_type`(이벤트 시점 상품유형 스냅샷 — REGISTER 지정값·MOVE/RECOVER 당시 배치 값·CORRECT 변경 후 값; RECOVER 스냅샷이 교체 집계 축) · `deal_code`(이벤트 시점 계약건 스냅샷(B-23) — REGISTER=새 배치 값·그 외=당시 배치 값, RECOVER 스냅샷이 딜별 교체 집계 축)
...
- 회수 사유는 StatusCode `DEVICE_RECOVERY_REASON`(value 행 삭제 불가 — value DEFECT→AS접수·리프레시센터 / LOST→분실 / DISPOSE→폐기 상태 매핑 `RECOVERY_REASON_CONDITION`, 2026-09-17), 거점은 StatusCode **`DEVICE_SITE`**(REFRESH_CENTER 리프레시센터 / HUB thynC Connected Hub — 2행 고정·설정 UI 없음, 마이그·`scripts/seed-device-registry.sql` 양쪽 INSERT, B-30), 계약 대조는 Σ계약완료 딜 `daewoong_device_count`(ECG hard·SpO2 soft)
...
- **v1.5 편입 (2026-09-08) — 미편입 모듈 일괄 편입 (신규 9키 + `vehicle.manage` 배선 확장)**: `as_receipt.admin`(종결 후 수정·타인/종결 후 삭제 — `canEdit/canDeleteAsReceipt`에 adminPerm 파라미터)·`stock_out.admin`(타인/종결 후 수정·삭제 — `canEditStockOutRequest` 동일 방식)·`voc.admin`(VOC접수 삭제)·`ticket.admin`(티켓 삭제·타인 코멘트 수정/삭제·지표 perOwner 열람)·`gateway_planner.access`(플래너 모듈 전체 접근 — 설정은 ADMIN 유지)·`hospital.admin`(병원 삭제·Drive 내보내기·Excel 가져오기 버튼)·`wiki.admin`(보호 페이지 삭제·AI 검색 제외·타인 댓글 수정/삭제)·`consultation.admin`(타인 상담이력 수정/삭제 — SEERS 축 AND 유지)·`ai_assistant.admin`(피드백 통계 열람·타인 세션 삭제 — 사용 자체는 SEERS 축). `vehicle.manage` 확장: 반납 취소·타인 예약 수정/삭제/반납 처리·타인 운행일지 수정/삭제·운전자 타인 지정(반납·운행일지). 전부 표준형 `ADMIN 이상 OR (USER 이상 + 권한)` 가산 — 본인/종결/소속/차단 축 보존, UI 게이트는 `/api/auth/me` `permissions[]` 또는 서버 컴포넌트 `hasPermission`으로 동반 확장. 편입 제외(등급 유지): SUPER_ADMIN 축(병원 업무이전·HIRA 동기화·메뉴/조직 관리), 설정 마스터 축(시공사·상태코드·티켓 마스터 등), 기기 마스터 5필드·회수 사유(설계 §8.1 의도적 제외), 계정 거버넌스(`/api/users`)
...
- **AsReceipt** (`as_receipts`): 기기 수리·교체(AS) 접수 — **8번째 티켓 도메인** (refType `AS`, 어댑터 `lib/ticket-domains/asReceipt.ts`) · `asCode`(AS-YYYYMM-NNNN) · `hospitalCode`(필수 FK) · `category`(FAULT 고장/LOST 분실 — CHECK, 2026-09-04 확정 2종) · 엑셀 A~Y 매핑 축: `receiptDate`·`reporterName`(고객명/카카오채널)·수거(`pickupMethod` PARCEL/VISIT·`pickupTrackingNo`·`pickedUpAt`)·`receivedAt`(입고)·`checkedAt`(확인일 O열, 입고 대조 2026-09-11)·`sheetDoneSynced`(시트 X열 마지막 기입 값 — 이벤트 기반 역기입, 2026-09-11)·`preReplace`(선교체)·**태그 3종**(`priorityRepair` 우선수리·`firmwareUpdate` 펌웨어 업데이트·`accessoryIncluded` 부속품 동봉 — 2026-09-15, 선교체와 함께 목록 '태그' 열)·발송지(`destType` HOSPITAL/OTHER·`destInfo`)·**회수지**(`pickupDestDiffers` 상이 체크·`pickupDestInfo` — 채널톡 인입 시 발송지 자동 복사, CX #13 2026-09-07)·`expectedShipDate`·`note` · 상태(`AS_STATUS` **단계형 8종**(2026-09-10 '처리중' 제거, 2026-09-11 '발송완료' 추가): 접수→OPEN/수거중·입고·발송·발송완료→IN_PROGRESS/보류→PENDING/**완료·취소→CLOSED** — 선교체·방문교체가 있어 단계 순서 강제 없음) · `resolvedAt` · `createdById`(등록자 — 담당 배정은 티켓 단독 소유) · `ticketId`(1:1) **수거방법 `pickup_method` CHECK: PARCEL/VISIT/NONE(수거없음 — 2026-09-18 마이그 `20260918120000_as_pickup_method_none`, 분실 등록 기본값 `defaultAsPickupMethod`)** **`combined_pack`(합포장 태그, 2026-09-19 마이그 `20260919120000_as_receipt_combined_pack` — 컬럼 + 수거 송장 정규화 표현식 인덱스)**
- **AsReceiptItem** (`as_receipt_items`): 기기 라인(1접수:N기기 — UNIQUE(receipt,serial)·CASCADE) · **입고 대조(2026-09-11)** `intakeState`(PENDING 대기/RECEIVED 정상입고/MISMATCH 미입고/EXTRA 미식별입고 — CHECK)·`receivedAt`(라인 입고일)·`receiptSerialNo`(치환 전 접수 시리얼)·`intakeSource`(RECEIPT/INTAKE — CHECK) — `serialNo`(정규화)·`deviceId`(device_units 소프트 연결, **NULL=미등록 라인** — 경고 후 허용·추후 백필)·`deviceKind`(미등록 라인 기기종류)·`wardName`·`symptom`(기기별 접수사유)·`processNote` · **라인 단위 결과** `outcome`(REPAIR_RETURN 수리반환/REPLACE 교체/LOST 분실종결/CANCELED/**NOT_RECEIVED 미회수**(입고 대조 예외, 2026-09-11) — CHECK) · 교체기(`newSerialNo`·`newDeviceId`) · 발송(`shipMethod`·`shipTrackingNo`·`shippedAt` — **부분 발송 지원**) · **수리완료(2026-09-17 — `device_condition_location_design.md` §5.4, outcome·헤더 전이·완료 판정에 불개입하는 제3축)** `repairedAt`(DATE, 서버 오늘 KST B-34)·`repairedById`(FK users SET NULL) — `canMarkAsLineRepaired`(정상입고 ∧ 결과 ∉ 분실·취소·미회수)인 라인만, 기기 `condition=REPAIRED`와 같은 tx(마이그 `20260917120000_device_condition_location`)
...
- **AS접수 라인 처리방법 초안 → 최종확정** (2026-09-14): 기기군 카드에서 라인 선택 → 처리방법(수리반환/교체/분실/취소) [처리방법 저장]은 **초안**(`draft_outcome`·`draft_new_serial_no`)으로만 기록되어 기기현황·티켓·시트에 영향이 없고 [지정 해제]·재지정으로 자유롭게 변경 가능. 발송정보(기기군 공통) 입력란은 처리방법 지정 아래로 이동, 확정·초안 발송 라인 모두에 적용. 3. AS상세내역 카드 하단 **[최종확정]**(기준일 입력)이 초안 전 라인을 한 번에 확정 — 이때만 기기현황(AS 해제·교체·회수) 기록, 확정 라인은 변경 불가, 전 라인 확정 시 '발송완료' 자동 전이(기존 규칙). 결과 열에 `(초안)` 점선 배지, 카드 헤더에 초안 수
...
- **URL 동기화**: `?view=hospital&hospital=&tab=list|history|wards|import&status=&model=&ward=&q=&page=&device=<id>` / `?view=devices&status=&model=&usage=&productType=&q=&page=&device=`(드로어 딥링크 공통; 구 `tab=coverage|events` 링크는 기본값으로 매핑). (전체 형상) 헤더: [Excel](활성 탭 기준 — 기기 목록/이력/커버리지) · 시리얼 조회(ACTIVE → 그 병원 + 드로어, RECOVERED → 마지막 병원 + '회수됨' 필터 + 드로어, 0건 → '원장에 없음' + 접두 일치 ≤10 + WMS 개체 ≤10) · 병원 콤보(계약완료 딜 보유 ∪ 원장 보유 사전 로드 — 2026-09-02 모집단 축소(구 상태 기반 제거)·커버리지 1요청 겸용 + '전체 병원 검색' 토글)
...
- **계약건(딜) 소프트 참조(2026-09-02 B-23)**: 배치 속성 `hospital_devices.deal_code`(FK 없음 — 딜 재적재 대비, 이벤트 시점 스냅샷 포함) — 등록·임포트에 '계약건' select(단일 계약완료 딜이면 자동 기본값, 딜 선택 시 상품유형이 딜에서 파생 — 명시 충돌 400 `선택한 계약건의 상품유형과 다릅니다`, 타 병원·없는 코드 409 `이 병원의 계약완료 딜이 아닙니다`), 교체는 구 배치 계약건 상속(소급 경로만 select). 병원 뷰 상단은 **계약건별 현황 표** `계약건 | 유형 | 심전계(도입/등록) | 산소포화도(도입/등록) | 혈압계(도입/등록) | 교체 건수`(모델 셀 2행째 'AS a · 누적 b' — AS진행중·누적 AS=교체 짝 수, 둘 다 0이면 숨김; +'(미지정)' 행·합계 ⓘ 팝오버, 계약 외 코드는 '(계약 외)' — 딜 재적재로 끊긴 코드 발견 경로), 목록 [열 더보기] '계약건' 열·인라인 계약건 select(`?deal=<code|none>`), 선택 바 [계약건 지정](bulk SET_DEAL — 기기마다 CORRECT), 드로어 헤더 인라인 변경(write), Excel '계약건' 열
- **계약 수량 = 딜 모델별 수량 단일 소스(2026-09-02 B-25, 당일 개정)**: 기대(도입) 수량은 `sales_deal_devices`(딜×모델, 딜 상세 '규모·계약' 카드 '도입 기기 수량')만 사용 — `daewoong_device_count` 폴백 **제거**(참고 표기 전용: 계약 팝오버 '대웅 디바이스수 합 n (참고)'). 수량 미입력 딜은 도입 '—' + ⓘ '모델별 도입 기기 수량 미입력 — 딜 상세에서 입력'(계약별 표·커버리지 계약 열 동일), 그 모델 행이 병원에 하나도 없으면 compare `none`(SpO2 'ECG 동수 가정' soft 제거 — 실측 행이 있을 때만 hard). 커버리지 '차이 있음' 필터·'차이 큰 순' 정렬은 expected 보유 병원만 대상(null은 뒤·미매치), 판매유형 배지는 수량 무관 딜 유형(`dealProductTypes`) 기준
- **AS진행중 플래그(2026-09-02 B-24)**: ACTIVE 배치의 표시 플래그(`as_started_on`·`as_ref_code`, 제3의 상태 아님) — 행 ⋯·드로어·선택 바 [AS 접수](구 'AS 표시' — 2026-09-02 용어 개정; 유지보수 코드 선택 소형 모달 → AS_OPEN 이벤트)/[AS 해제](AS_CLEAR), **교체·회수 시 자동 해제**(이벤트 없음), 이력에 'AS 접수/해제' 행. 개체 상태 표시는 `사용중`/`AS진행중`(warning)/`회수됨`('배치 중'은 집계 문구 전용), 'AS진행중만' 체크는 전체 모드(`?as=1`), 병원 요약 'AS진행중 n' 칩(`asInProgress`)·계약별 표 모델 셀 AS·누적(`asByModel`·`replacementsByModel`), Excel 상태 열 갱신 + 'AS 접수일' 열
- **기기 상태·위치 축 (2026-09-17 — `projects/device_condition_location_design.md`, HDR D11 개정 B-36)**: 배치(어느 병원 몫인가 — 불변)·**기기 상태**(실물 6종 사용중/AS접수/수리완료/출고 전/분실/폐기 + 미확인)·**위치**(병원/리프레시센터/thynC Connected Hub/—) 3축 분리(B-26 유닛 속성). 목록 기존 '상태' 열 헤더 → **'배치'**(값 사용중/AS진행중/회수됨 불변, A-3) + **'기기 상태'·'위치'** 열(compact 기본 열에는 '기기 상태'만) · 필터 `?condition=<6종|none>&location=HOSPITAL|REFRESH_CENTER|HUB|none`(병원 뷰 compact·전체 모드 필터 행과 [디바이스] 뷰에 인라인 셀렉트 — 회수 목록 `condition=REPAIRED&location=REFRESH_CENTER` = 교체품 가용 v1 근사 I-5) · 드로어 헤더 배치 배지 뒤 기기 상태 배지 + `위치: 리프레시센터 (09-14~)` · 드로어 액션(배치 RECOVERED/없음): **[수리완료]**(AS접수/미확인 → REPAIR_DONE)·**[수리완료 해제]**(CORRECT)·**[폐기]**(write USER+ A-5 — confirm + memo 필수 프롬프트 → SCRAP)·**[위치 이동]**(버튼 ▾ 플로팅 패널에서 목적지 거점 선택 → SITE_MOVE); 배치 ACTIVE ∧ 위치 거점 ∧ 사용중이면 **[병원 반환]**(SITE_MOVE → 배치 병원, 라인 취소·수동 AS 해제 후 실물이 센터에 남은 I-4 예외 해소), AS접수/수리완료면 안내 '미종결 입고 라인 — 실물 반환은 AS 상세에서 확정하세요'. 드로어 [수리완료]/[해제]/[폐기]는 그 기기의 입고 라인(`canMarkAsLineRepaired`, 종결 접수 포함)에 `repaired_at` 동기화 + 접수 비고 이력(같은 tx, 이벤트 ref 없음). 이벤트 행: '입고(AS-…)'·'수리 완료'·'폐기'·'위치 이동 A → B' + REGISTER/RECOVER/AS_*/CORRECT 행에 `→ 사용중·병원` 스냅샷 문장(`app/devices/_components/deviceDisplay.ts` 문장화 헬퍼 단일 소스 — 드로어·이력 탭·이벤트 export·PATCH 감사 공용). **암묵 전이**(공용 헬퍼 — 단건·일괄·교체·임포트·WMS 출고 동일): REGISTER → 사용중·병원(폐기 기기 409 '폐기된 기기입니다 — 정정 후 등록하세요', 수리완료가 아닌 재등록은 경고 '수리완료 체크 없이 재사용'), RECOVER 사유 DEFECT → AS접수·리프레시센터(A-4 스냅샷 `note` '입고 미확인') / LOST → 분실·위치 없음 / DISPOSE → 폐기 / 그 외 유지, AS_OPEN → AS접수, 수동 AS_CLEAR → 사용중(위치 유지 + 경고 '미종결 입고 라인(AS-…) 있음'). 관리 보정 PATCH `{condition, location}`(admin OR device.admin — **출고 전(PRE_SHIP)의 v1 유일 진입로** A-6; Hub 입고 폼·Hub/센터 재고 목록·교체품 가용 카드·일괄 수리완료/폐기·거점 설정 UI는 Phase 2). 취소·정정 규약(§8.2 — 단일 정렬 기준 id): 스냅샷 이벤트는 id 더 큰 스냅샷이 남아 있으면 409 '최근 이벤트를 먼저 취소하세요', 신규 4종은 `changes.before` 복원, 배치 축 이벤트 취소 후 유닛 값 재도출(B-32), 배치 축 스냅샷 이벤트의 소급 삽입·occurredOn 역전은 409. Excel 기기 목록 '기기 상태'·'위치' 열, 이벤트 export 4종 요약 + 스냅샷 문장('내용' 폭 48). 백필 `scripts/backfill-device-condition.mts`(`--dry` 규칙별 건수·목록 리포트 / `--rehearse` 적용 후 롤백(리허설) / `--apply` 단일 tx — `/api/health`의 `eventTypes`·`buildCommit`으로 실행 중 서버 빌드 확인, DEV만 `--skip-health-check`). DEV --rehearse 실측: 사용중·병원 26,023 / AS접수·병원 734 / AS접수·센터 130 / 분실 128 / 미확인·센터 1,340 / INTAKE 백필 1,470. 스모크 `scripts/smoke-device-registry.mts` [1e-1]~[1e-11](123항목, 전체 686 pass)
...
- 목록 컬럼(2026-09-15 개편): 접수번호·병원·**접수 기기상태**·구분·**기기**(축약 배지 `ECG n`·`SpO2 n`·`ETC n`, 종결분 `/n` 흐리게, 툴팁에 기기별 상세 — `summarizeAsItemsByGroup`)·**유형**(라인 기기의 원장 배치 상품유형 일반/라이트 — 혼재 시 둘 다 배지, 구기기 배치 회수 후엔 교체기 배치 기준)·상태·접수일·**입고일**(2026-09-16 — 라인 입고일 중 최신, 부분 입고·복수 날짜면 `(n/m)`+툴팁, 라인 입고일이 없으면 접수 헤더 입고일)·**발송일**(라인 발송일 중 최신, 부분 발송·복수 날짜면 `(n/m)`+툴팁)·**발송 송장번호**(라인 송장 중복 제거, 여러 개면 `+n`+툴팁)·**태그**(선교체·우선수리·펌웨어 업데이트·부속품 동봉 색 배지 — 4개가 한 줄에 들어가는 고정 폭 27rem) — 수거·선교체·발송지·완료일·등록자·티켓 코드·담당 컬럼은 제거(상세에서 확인). **태그 필터** 칩(복수 선택 = 모두 켜진 접수, URL `tag` 복수 파라미터) · **'확인필요만'** 체크(접수 기기상태 배지와 같은 정의 — `needsCheck=1`, 2026-09-15) · **페이징**(2026-09-16, `app/components/ui/Pager`): 첫/마지막·±5페이지·한 페이지 이동·번호 직접 입력(Enter/blur)·전체 건수, 페이지는 URL `page`와 동기화 · **컬럼 정렬**(2026-09-16 — 접수번호·병원·구분·상태·접수일·입고일·발송일 헤더 클릭 asc→desc→기본, 서버 정렬, URL `sort/dir` 동기화; 계산 컬럼은 정렬 없음) · **날짜 필터 개선**(2026-09-16 — 접수일·**입고일**(신설)·발송일 3개를 `app/components/ui/DateRangeFilter` 팝오버 달력으로 교체: 시작·종료일 클릭(호버 미리보기)·프리셋(오늘·최근 7일·최근 30일·이번 달·지난 달)·직접 입력·× 해제, 버튼에 현재 범위 요약) · **필터 초기화** 버튼(필터가 하나라도 켜지면 표시) · **검색 항목 드롭다운 + 쉼표 복수 키워드**(2026-09-18 — 통합검색·병원명·시리얼번호·접수번호·송장번호·담당자, `AS_SEARCH_FIELDS` 카탈로그; 검색어를 `,`로 나눠 하나라도 맞으면 포함 — 송장번호 여러 개 한 번에 조회, URL `field`(통합이면 생략)·`q` 동기화) · **[검색]/Enter는 검색어가 같아도(공란 포함) 목록·요약 즉시 재조회**(2026-09-18 — 공란 [검색] = 새로고침) · **체크박스 + 상태 일괄변경**(2026-09-21 — 쓰기 권한자에게 행 왼쪽 체크박스·헤더 전체선택(현재 페이지), 선택이 있으면 표 위에 파란 바 `n건 선택 | 상태 선택 → [적용]`, confirm 후 `POST /api/as-receipts/bulk-status`; 결과(변경·이미 같은 상태·제외 사유)를 상단 배너로, 페이지·필터 이동 시 화면 밖 선택 자동 해제, 최대 100건) · **상세 [← 목록] 버튼**(2026-09-21 — 목록이 마지막 조회 쿼리스트링을 sessionStorage `as_receipts_list_qs`에 보관, 상세 헤더 [← 목록]·삭제 후 복귀가 `asListHref()`로 필터·정렬·페이지 그대로 복원)
- **상세 저장 후 갱신 (2026-09-18)**: 헤더 [수정] 모달·접수정보/AS상세 저장·입고처리·처리·확정 등 모든 성공 응답 뒤 `router.refresh()` + 상세 API 재조회(`cache: 'no-store'`)에 더해 `reloadSeq`를 기기군 카드(GroupCard) key에 넣어 **카드를 리마운트** — 발송정보·선택·초안 입력 등 카드 로컬 상태가 서버 값으로 리셋된다(종전엔 props 초기값으로만 잡혀 저장 뒤 구 값이 남던 문제)
- **접수 태그 (2026-09-15)**: `as_receipts` 불리언 5종(`pre_replace`·`priority_repair`·`firmware_update`·`accessory_included`·**`combined_pack` 합포장(2026-09-19 — 수동 체크 + 수거 송장번호 기입·변경 시 같은 번호(영숫자 정규화)의 다른 접수가 있으면 양쪽 자동 켬, 자동 끔 없음. 이벤트 기반이라 주기 검사 없음 — `lib/asReceiptSearch.syncCombinedPackByTracking`, 정규화 표현식 인덱스 `as_receipts_pickup_tracking_norm_idx`. 기존 데이터는 `scripts/backfill-as-combined-pack.mts --apply` 1회)**) — 카탈로그·라벨·색·컬럼 매핑은 `lib/asReceiptShared.ts` `AS_TAGS`/`AS_TAG_FIELDS`/`AS_TAG_BADGE_CLS` 단일 소스. 등록·수정 모달과 상세 2. 접수정보 체크박스, 목록 태그 열·필터, Excel 열, 타임라인 diff에 공용
- **접수 기기상태**(`registryTags`, `summarizeAsRegistryTags`·`asReceiptDeviceStateLabel`): 접수 병원과 라인 기기의 **현재** 원장 배치를 조회 시 실시간 대조(미종결 라인만 — 종결 라인은 교체·분실 회수가 정상이라 제외). 목록은 **정상 / 확인필요** 2단계(확인필요 툴팁에 태그별 대수, 미종결 라인 없으면 '-') + **취소 접수는 '취소'**(2026-09-21 — 상태 '취소'(`isAsCanceledStatus`, 완료와 같은 CLOSED 매핑이라 이름 기준)면 원장·입고 검토 대상이 아니므로 확인필요 대신 회색 '취소' 배지, `needsCheck` 필터에서도 제외. 목록 행은 접수번호를 제외한 전 열 취소선+흐리게), 라인별 태그는 **상세 시리얼 옆 배지**(`classifyAsRegistryLine`, 상세 API가 라인에 `registryTag` 부가) — `타병원`(다른 병원 ACTIVE, 병원명 병기)·`회수`(RECOVERED)·`미배치`(개체만 있고 배치 없음)·`미등록`(시리얼 없음) 4종 + **`중복접수`(2026-09-18 — 같은 시리얼의 미종결 라인이 다른 AS접수에도 있음, 병원 무관; 배치 태그와 별개 축이라 한 라인이 둘 다 가질 수 있음. 상세 배지에 상대 접수번호 링크, 목록 툴팁·`needsCheck` 필터에 합류. 원장 정합 확인 패널 대상은 아님 — `lib/asReceiptSearch.findOpenLinesBySerial`. **상대 접수 헤더가 종결(완료·취소)이면 라인이 열려 있어도 제외**(2026-09-19, `AS_OPEN_RECEIPT_WHERE`·needsCheck SQL 동일))**. 종결 라인은 기존 등록 시 deviceId 기준 배지(미등록·회수·타 병원·AS진행중) 유지
...
- 상세(2026-09-11 카드 재구성 — 시트 입력 주체 기준): **1 공통정보**(병원·구분·상태·담당/티켓·등록자·등록일시·완료일·기기 수) → **2 접수정보**(접수자 입력, 시트 A~M·S·T — **접수일(2026-09-18 인라인 date 입력, 필수 — 시트 역기입 없음)**·고객명(2026-09-19 인라인 입력)·수거방법(택배수거/방문수거/**수거없음** — 2026-09-18 추가, 분실 접수는 등록 시 자동 '수거없음'·채널톡 인입도 동일)·수거 송장·수거일·발송지 구분/정보·회수지, 인라인 저장) → **3 AS상세내역**(AS담당자 입력, 시트 N~X — 입고일·예상 출하일 + **기기군별 카드 3-1 심전계 / 3-2 산소포화도 / 3-n 기타**(`asDeviceGroupOf`: 원장 모델명 또는 미등록 기기종류로 접음): 라인 표(시리얼·병동·증상·**처리내용 라인별 인라인 저장**·결과·교체기·발송) + **기기군 공통 발송정보**(발송방법·송장·발송일 한 번 입력 — 처리 실행 시 선택 라인에 적용, [발송 라인 n대에 적용]으로 기발송 라인 일괄 정정 `POST /ship-info`) + 라인 처리(결과·처리일·교체기 시리얼, **부분 발송 지원**)) → 2. 접수정보 하단에 **태그 체크박스 4종 + 비고 `note`**(2026-09-15 — 5번 카드에서 이동, [접수정보 저장]에 포함). 처리 시 기기현황 즉시 기록(교체 = 구기기 회수+교체기 등록, 분실 = LOST 회수), 전 라인 종결 → 접수 **'발송완료'** 자동(2026-09-11 개정, 티켓 IN_PROGRESS 유지) → **4. 기기등록** 카드 [완료](`POST /complete`, 미종결 라인 0 필수)로 최종 '완료'·완료일·티켓 CLOSED → **5. 타임라인**(2026-09-15 — `GET /timeline`: 감사로그(등록·접수 수정 필드 diff·입고처리·입고 확인·라인 최종확정·발송정보·원장 확정·완료·리오픈) + 연결 티켓 로그(생성·상태 전이·담당·댓글) 합성, 최신순. 초안 저장은 감사로그가 없어 미표시). 처리일이 AS 표시 시작일보다 앞서면 표시가 남는다는 경고 반환
- **입고 대조 (2026-09-11 — `as_work_design.md` §14)**: 3번 카드 [입고처리] — AS담당자가 실물 시리얼(줄 단위)+입고일(N)·확인일(O) 입력 → 접수 라인과 대조(**누적 실행**): 일치 → 라인 `정상입고`, 접수됐으나 없음 → `미입고`, 접수에 없음 → `미식별입고` 라인 생성(미등록이면 시리얼 접두 A/P/B로 기기종류 추정). 헤더 입고일(최초)·확인일 갱신, 상태 '입고' 자동. **접수자 확인**(USER 이상 전원): 미입고 → 치환(미식별입고 시리얼로 교체, 원 시리얼 `receiptSerialNo` 보존·AS 표시 이전)·정상입고 확정·**미회수**(결과 `NOT_RECEIVED`, 코멘트 필수, 종결 간주) / 미식별입고 → 신규 라인 편입·삭제. 확인 이력은 비고에 자동 추가. **시리얼 보정 (2026-09-15)**: 원장 정합 확인 패널의 각 라인에 [시리얼 보정] — 채널톡·시트 인입 오타(예 `P018330사용중/삭제X`)를 담당자가 즉시 고침(`POST /correct-serial`). 실제 미등록·타병원·회수 기기는 종전대로 [확정]. **처리 게이트**: 미입고·미식별입고 라인은 결과 확정 409(대기 라인은 허용 — 방문교체·선교체·기존 접수 호환). **종결 라인 사후 입고 (2026-09-15)**: 선교체·방문교체처럼 처리(발송)가 입고보다 앞선 라인은 결과 확정 뒤 대기(PENDING) 상태로 남으며, 입고처리에서 시리얼이 일치하면 정상입고·라인 입고일이 기록된다(경고로 '처리 완료된 라인의 사후 입고' 안내, 상태·결과·기기현황 불변). 미회수로 종결된 미입고 라인만 자동으로 뒤집지 않는다. 입고 대조 라벨은 원장 정합 태그와 별개 축이며, 목록 '접수 기기상태'는 둘 중 하나라도 있으면 '확인필요'(툴팁 사유 구분). 시트 N·O 역기입, Excel 입고상태·라인입고일·접수시리얼 컬럼
- **원장 정합 확정 (2026-09-11 — `as_work_design.md` §15)**: 원장 태그(타병원·회수·미배치·미등록)가 붙은 미종결 라인을 기기군 카드 하단 '원장 정합 확인' 패널에 나열, [확정] 시 `registerDevicesIn` 1회로 미등록 → 신규 등록(모델 접두 추정·선택) / 회수·미배치 → 재등록 / 타병원 → 이관(TRANSFER opt-in) 하여 접수 병원 ACTIVE 배치 + 라인 `deviceId` 연결 + AS 표시. 상품유형은 원장 규칙(혼합 병원이면 선택 필수). 비고에 `[원장확정 …]` 이력. `POST /api/as-receipts/[id]/registry-confirm`(USER 이상)
- **수리완료 체크·기기 상태 연동 (2026-09-17 — `projects/device_condition_location_design.md` §6.1·§7.2·§7.3, `as_work_design.md` §17)**: 3번 카드 라인 표 '처리내용'과 '결과' 사이 **'수리완료' 체크박스**(입고된 라인만 D5 — `canMarkAsLineRepaired`: 정상입고 ∧ 결과 ∉ 분실·취소·미회수; 결과 확정 라인(선교체)·**종결 접수도 체크 가능** A-2, 권한 VIEWER 제외 — `canEdit`·`isTerminal`과 분리) → `POST /repair-done` — 라인 `repaired_at/by`(서버 오늘 KST) + 기기 상태 REPAIRED(REPAIR_DONE, ref AS) / 해제는 confirm 1회 + CORRECT(AS접수 복귀) + 비고 `[수리완료 09-17 홍길동] P018330`(재체크는 멱등·경고 '변경 사항 없음'). 카드 헤더 칩 `수리완료 n/m`(m = 체크 가능 라인, n<m amber·n=m green, m=0 숨김), 시리얼 셀에 기기 상태 소형 배지(수리완료·폐기·분실 — 라벨은 `deviceConditionLabel` 단일 소스, 톤은 `AS_LINE_CONDITION_BADGE_CLS`) + 위치 툴팁(접수 병원/병원명/거점), 수리완료 셀 **[폐기]**(회수된 라인 기기만 — confirm + memo 필수 → `POST /scrap-line`, 라인 repaired_at NULL·비고 `[폐기 …] memo`·경고 배너 '{시리얼} 폐기 처리됨'). 목록 '기기' 배지에 `수리 n/m` 병기(`summarizeAsItemsByGroup`), Excel '수리완료일' 열, 타임라인 감사 접미어 수리완료/수리완료 해제/폐기. **AS 흐름 훅**(`lib/asReceiptService.ts` §7.3): 접수 등록 AS_OPEN → AS접수(병원 유지) · 입고처리/입고 확인(정상입고 확정·미식별 편입·치환 후 기기) → INTAKE 리프레시센터(결과 NULL·교체 라인만, ref별 1회 기록 B-37) · 수리반환 확정·미회수·라인 취소·라인 제거·접수 삭제·시리얼 보정·치환 전 기기 → `setUnitInUse`(사용중 복귀 — **되돌림 게이트 `ownsDeviceState`**: 그 기기의 마지막 스냅샷 이벤트가 이 접수 ref이거나 비AS ref일 때만, 타 접수면 경고 '다른 접수(AS-…)가 최근 상태를 기록 — 유지'; 이 접수가 켠 플래그면 AS_CLEAR(처리일 < 표시 시작일이면 업무일자를 표시 시작일로 클램프 + 경고), 아니면 CORRECT 폴백 memo '수리반환 확정/라인 취소/미회수 확정/라인 제거/접수 삭제/시리얼 보정/시리얼 치환 AS-…'; 라인 취소는 위치 유지 → 기기현황 [병원 반환]) · 교체 확정 구기기 RECOVER(DEFECT) → AS접수·리프레시센터(A-4) · 분실종결 → 분실 · 체크된 라인이 분실·취소·미회수로 확정되면 repaired_at NULL + 비고 · 원장 확정·시리얼 보정 신기기·병원 변경 재생성 라인은 INTAKE(occurredOn=라인 입고일)·REPAIR_DONE(occurredOn=repaired_at) 재적용(재생성 시 입고 상태·입고일·접수 시리얼·수리완료 필드 보존 — 기존 유실 결함 동반 수정). **종결 접수 사후 입고 조건부 허용**(A-2 짝): 입력 시리얼 전부가 결과 '교체' 라인(대기·정상입고)과 일치할 때만(정상입고 재입력은 무변경 통과, 전환 0건이면 비고 없이 경고 '변경 사항 없음', 불일치 시리얼 1건이라도 있으면 400·미식별 라인 생성 없음) — 헤더 상태·입고일·확인일 불변. 채널톡 인입은 동시 변경(`RegistryTxAbort`) 시 행을 '대기'로 두고 다음 틱 재시도(24h 한도). 스모크 '▶ 수리완료' [C-1]~[C-16]·[C-I6]
- **리오픈 (2026-09-11)**: 완료·취소 접수 헤더의 [리오픈](USER 이상) — 사유 입력(비고 `[리오픈 …]` 이력) → 비종결 상태로 복귀(정상입고 라인 있으면 '입고', 없으면 '접수')·완료일 해제·티켓 재오픈. 라인 결과와 기기현황 이벤트는 되돌리지 않음(라인 되돌리기는 별도 결정). 시트 X열은 다음 틱에 '미완료' 1회 기입(§7.1 이벤트 기반 개정)
...
- **인벤토리 완전 분리 (Phase 10 재설계)**: 인벤토리는 **대웅제약재고 / 평가용재고 / 판매용재고** 3종(`/settings/inventories`에서 추가·병원 연결·활성 편집). **품목·위치(창고)·재고·전표·개체 전부 인벤토리에 귀속** — 같은 MC200M-T라도 인벤토리마다 별도 품목(별도 코드)으로 등록해 완전 독립 관리. **이관(TRANSFER) 기능 폐지**(과거 전표는 '이관(구)'로 이력 표시만, 취소 불가) — 인벤토리 간 이동이 필요하면 A 출고 + B 입고로 각각 처리
  - **자재 현황** (`/inventory`, 전 로그인 조회): **인벤토리별 카드 섹션**(탭 없음) — 섹션 헤더에 인벤토리명·품목 수·총 수량·위치 수 + **입고/출고/이동 버튼 1세트**(품목은 모달에서 검색·선택, 행별 입출고 버튼 없음). 검색·분류 필터는 전 섹션 공통, **Excel 다운로드**. **인벤토리 접기/펴기**(2026-09-11): 섹션 헤더 클릭으로 토글(▶ 표시, 접혀도 요약은 표시), 필터 바 '모두 접기/펼치기', 접힘 상태는 localStorage(`inventory.collapsedInventories`) 영속. 검색·분류 필터 중에는 결과가 보이도록 전부 펼침(토글 비활성)
...
- **회수(반품) 입고 — 미등록 시리얼 확인 후 신규 등록 (2026-09-11)**: 회수 유형 입고에 시스템 출고 이력이 없는 시리얼(자재관리 도입 전 출고분)이 섞이면 API가 409 `code=UNKNOWN_RETURN_SERIALS`·`unknownSerials`로 반환 → 모달이 목록을 보여주고 '신규 등록하고 회수 확정' 클릭 시 `allowUnknownReturn=true`로 재요청해 해당 시리얼을 신규 개체(IN_STOCK)로 생성. 오타는 확인 단계에서 걸러짐. 전표 취소 시 이 전표에만 연결된(신규 생성) 개체는 삭제, 기존 개체는 OUT 복귀
...
- **계정 검색** (2026-09-11): 탭 아래 검색창 — 이름·이메일·연락처·부서·등급/역할명 부분 일치(현재 탭 내, 클라이언트 필터), 결과 수 표시·지우기
...
- `/settings/roles` 한 페이지 3구역: ① 역할 목록(추가·이름/설명 수정·활성·순서·삭제) ② 권한 할당(카탈로그 모듈별 체크박스, 토글 즉시 저장) ③ 멤버 할당(활성 사용자 검색 추가/회수). 멤버 추가 모달(2026-09-11 수정): 추가 후 검색어·포커스 유지, 백드롭은 mousedown+click 모두 백드롭일 때만 닫힘(입력창 드래그 선택 후 바깥에서 놓아도 안 닫힘), 검색 응답 순서 보정, `router.refresh()`는 모달 닫을 때 1회
...
| GET | `/api/health` | 무인증 생존 확인 `{ok:true}` — 사이니지 자동 복구 폴링용 (DB 미접근) · **2026-09-17** `eventTypes`(DEVICE_EVENT_TYPES)·`buildCommit`(next.config `env.GIT_COMMIT` — 빌드 시 `git rev-parse --short HEAD`를 번들에 인라인, git 없으면 null) — 백필 `scripts/backfill-device-condition.mts --apply`의 실행 중 서버 가드 |
...
| GET  | `/api/devices/summary` | 전역 커버리지 표 + totals — 모집단 = 계약완료 딜 보유 ∪ 원장 보유(2026-09-02) (`?page&limit≤1000&filter=all\|unregistered\|diff\|complete&q&sort=diff\|name\|lastEvent`) |
| GET  | `/api/devices/summary/export` | 커버리지 xlsx (캡 1,000행) |
| GET  | `/api/devices/units` | 기기 목록 (`?hospital&model&ward=<id>\|unassigned&status=active\|recovered\|all&q&wms=linked\|unlinked\|in_stock&usage=SALE\|EVAL\|none&productType=일반\|라이트\|none&deal=<딜코드>\|none&as=1&**condition=IN_USE\|AS_WAITING\|REPAIRED\|PRE_SHIP\|LOST\|SCRAPPED\|none&location=HOSPITAL\|REFRESH_CENTER\|HUB\|none**(2026-09-17 기기 상태·위치 축)&page&limit≤500&sort&idsOnly=1`(≤2,000)) |
| GET  | `/api/devices/units/[id]` | 개체 상세 + 전체 이벤트 + WMS + 상대 기기 (드로어) — 2026-09-17 `device.condition/conditionChangedOn/locationSiteValue/locationHospitalCode/locationChangedOn` + 상태·위치 축 이벤트(changes 스냅샷) |
| PATCH | `/api/devices/units/[id]` | memo(write) / 용도 `usageTypeId`·상품유형 `productType`·계약건 `dealCode`(계약완료 딜 아니면 409, write → CORRECT) / 식별 보정 모델·시리얼·MAC·닉네임(admin → CORRECT 이벤트, 유니크 409) / **기기 상태·위치 보정**(2026-09-17 §8.1) `condition`(6종\|null 미확인)·`location`(`{kind:'HOSPITAL'\|'SITE'\|null, code}` 또는 null — 거점 value·형태 오류 400)(admin OR device.admin → CORRECT, I-1·I-3·전이표 검증, 감사 라벨 '기기 상태 보정'/'위치 보정' 문장화 값 — **출고 전(PRE_SHIP) v1 진입로** A-6; 배치 없는 유닛 허용, 응답 `device`는 배치 행 있을 때만·없으면 null) |
| POST | `/api/devices/units/[id]/move` · `/recover` | 병동 이동 / 회수 (write — 같은 병동 400, 사유 없음 400, 이미 회수 409) |
| POST | `/api/devices/units/[id]/as-open` · `/as-clear` | 'AS진행중' 표시 / 해제 (write, B-24 — ref는 유지보수 코드 권장(`as_ref_code`), 이미 접수/접수 없음·회수됨 409, 교체·회수 시 자동 해제, audit 'AS 접수'/'AS 해제') — 2026-09-17 AS_OPEN → 기기 상태 AS접수, 수동 AS_CLEAR → 사용중·위치 유지(+경고 '미종결 입고 라인 있음'), 업무일자 이후 스냅샷 배치 축 이벤트가 있으면 409 소급 차단 |
| POST | `/api/devices/units/[id]/repair-done` · `/repair-undo` | 드로어 [수리완료]/[수리완료 해제] (write, 2026-09-17 기기 상태·위치 축 §7.1 — AS접수/미확인 → 수리완료 REPAIR_DONE / 수리완료 → AS접수 CORRECT memo '수리완료 해제'; 사용중 409 '사용중 기기는 수리완료 처리할 수 없습니다', 이미 대상 값이면 `changed:false`. **라인 동기화**: 같은 tx에서 그 기기의 입고 라인(`canMarkAsLineRepaired`, 종결 접수 포함)에 `repaired_at/by` 기록(done — NULL인 라인만)/해제(undo) + 접수 비고 `[수리완료 …]`/`[수리완료 해제 …] (기기현황)`. 응답 `{changed, event, device(배치 행 없으면 null), before, after, lines, warnings}`, audit '{라벨} 수리 완료'/'수리완료 해제') |
| POST | `/api/devices/units/[id]/scrap` · `/location` | 드로어 [폐기] `{ memo(필수), occurredOn?, ref? }`(write USER+ — A-5 'AS 업무 권한과 동일': 배치 ACTIVE 409 '배치 중 기기는 먼저 회수하세요'(I-3), 분실 409, memo 없음 400 → 폐기·위치 없음(SCRAP) + 입고 라인 `repaired_at` 전부 NULL + 대상 접수 비고 `[폐기 …] memo`, audit '{라벨} 폐기') / [위치 이동]·[병원 반환] `{ to: 'REFRESH_CENTER'\|'HUB'\|'HOSPITAL', occurredOn?, memo? }`(write — 배치 RECOVERED/없음은 현재 위치 무관 거점만(HOSPITAL 409), 배치 ACTIVE는 HOSPITAL(배치 병원 반환)·사용중만 — AS접수/수리완료 409 '미종결 입고 라인 — AS 상세에서 확정하세요', 미확인 409 '관리 보정으로 상태를 지정한 뒤 반환하세요', 거점 이동 409 '먼저 회수' → SITE_MOVE(상태 유지), 같은 위치면 `changed:false`, 분실·폐기 409. audit '{라벨} 위치 이동'/'병원 반환') |
| POST | `/api/devices/units/bulk` | 일괄 이동·회수·상품유형·계약건 지정 `action=MOVE_WARD\|RECOVER\|SET_PRODUCT_TYPE\|SET_DEAL` (write — 같은 병원 ACTIVE만 단일 tx, 타 병원·RECOVERED 섞이면 409, 이미 대상 값은 `skipped[]`; SET_PRODUCT_TYPE/SET_DEAL은 기기마다 CORRECT) |
...
| GET  | `/api/devices/events/export` | 이력 xlsx (캡 10,000행 — 2026-09-17 신규 4종 요약 + 상태·위치 스냅샷 문장 병기, '내용' 열 폭 30→48) |
| PATCH | `/api/devices/events/[id]` | 이벤트 인플레이스 정정 (admin — occurred_on·memo·reason·ref·to/from ward, fold 재검증 409 — 2026-09-17: 배치 축 스냅샷 이벤트(REGISTER·RECOVER·AS_OPEN·AS_CLEAR)의 occurred_on 정정이 같은 유닛의 다른 배치 축 스냅샷과 id 순↔일자 순 역전을 만들면 409 '업무일자 순서와 기록 순서가 어긋나 정정할 수 없습니다 — 최근 이벤트를 먼저 취소하세요'(`assertNoAxisInversion`); 신규 4종은 occurredOn·memo·ref만) |
| DELETE | `/api/devices/events/[id]` | 마지막 이벤트 취소 (admin — LIFO, 교체·이관 짝 동시 취소 — 2026-09-17 §8.2: 스냅샷 이벤트는 같은 유닛에 id 더 큰 스냅샷이 남아 있으면 409(`assertNoLaterSnapshot`), 신규 4종·상태 CORRECT는 `changes.before` 복원, 배치 축 이벤트 취소 후 유닛 상태·위치 재도출(① 남은 id 최대 스냅샷 after → ② 취소 이벤트 before → ③ 배치 rebuild 파생, B-32) + 축별 changed_on 재산출, I-3·I-4 위반은 `warnings`) |
| GET  | `/api/devices/export` | 기기 목록 xlsx (units 필터 동일, 캡 10,000행, 초과 400 — 2026-09-17 '상태' 열 뒤 '기기 상태'·'위치' 열) |
...
| Method | Endpoint | 설명 |
|--------|----------|------|
| GET | `/api/as-receipts` | 목록 (statusId **복수 가능**·category·hospitalCode·접수일 기간·**tag 복수(AND, 2026-09-15)**·**overdue=1**(접수 2주 경과 미종결 — summary.overdue2w와 동일 정의)·**needsCheck=1**(접수 기기상태 '확인필요' — 미종결 라인의 원장 정합 태그 또는 입고 대조 미입고·미식별입고 또는 **중복접수**(2026-09-18), raw SQL — **상태 '취소' 접수 제외**(2026-09-21))·q + **field**(2026-09-18 검색 항목: `all` 통합(접수번호·고객명·병원명·시리얼·송장·담당자) / `hospital` / `serial` / `code` / `tracking`(수거·발송 송장 — DB 값·검색어 양쪽을 영숫자만 남겨 대소문자 무시 비교하므로 `CJ 2609`·`2609`·`cj2609` 모두 `CJ 26096748…` 매치) / `owner`(연결 티켓 담당자 이름); 검색어는 **쉼표로 복수 키워드(OR)** — `lib/asReceiptSearch.buildAsReceiptSearchOr` 단일 소스, export 동일)·**receivedFrom/To**(입고일 — 라인 receivedAt 또는 헤더 receivedAt, 2026-09-16)·shippedFrom/To(발송일 — 라인)·**sort/dir**(2026-09-16: asCode·hospital·category·status(order)·receiptDate는 Prisma orderBy, receivedAt·shippedAt은 라인 최신 날짜 집계라 id 전량 조회 후 JS 정렬·슬라이스, 빈 값 항상 뒤)·페이징) |
...
| GET | `/api/as-receipts/[id]` | 상세 (기기 라인·원장 배치 상태·연결 티켓 포함 — 2026-09-17 라인 `repairedAt/repairedBy{id,name}` + `device.unit{condition, locationSiteValue, locationHospitalCode, locationHospitalName}`) |
| GET | `/api/as-receipts/[id]/timeline` | 타임라인(2026-09-15) — 감사로그+티켓 로그 합성 이력 `{ events[] }` 최신순. 로그인 전원 |
| PUT | `/api/as-receipts/[id]` | **수거 송장번호 최초 기입 시 상태 '수거중' 자동**(2026-09-15 — 명시 statusId 없을 때·'접수' 단계일 때만, 응답 `autoPickup`+경고 문구) ·  수정 — 헤더·진행 기록·상태(어댑터 동기화)·라인 편집(종결 라인 제거 400, 제거 라인 플래그 해제·추가 라인 AS 표시) |
| DELETE | `/api/as-receipts/[id]` | 삭제 — 티켓 동반, 이 접수가 켠 AS 표시 해제(이벤트 보존) |
| POST | `/api/as-receipts/bulk-status` | 상태 일괄변경 (2026-09-21) — `{ ids[], statusId }` ≤100건. 접수별 PUT 단건과 같은 규칙(`canEditAsReceipt`·`asStatusChangeData` statusChangedAt/완료일 자동·어댑터 티켓 동기화·감사·알림), 접수별 개별 트랜잭션 → `{ status, updated[], unchanged[], skipped[{asCode,reason}] }`. VIEWER 403 |
| POST | `/api/as-receipts/[id]/resolve-items` | 라인 결과 확정 — 수리반환/교체/분실종결/라인취소 + 부분 발송, 전 라인 종결 시 완료 자동 |
| POST | `/api/as-receipts/[id]/draft-lines` | 라인 처리방법 초안 저장/해제 (2026-09-14) — `{ lines: [{itemId, outcome|null, newSerial?}] }`, 기기현황·티켓·시트 영향 없음, 확정 라인 409 (USER 이상) |
| POST | `/api/as-receipts/[id]/confirm-lines` | 3. AS상세내역 [최종확정] (2026-09-14) — 초안 전 라인을 한 트랜잭션으로 확정(resolveAsLines 경유), `{ effectiveDate? }` 기준일, 발송 라인은 라인에 기입된 발송일 우선 (USER 이상) |
| POST | `/api/as-receipts/[id]/ship-info` | 발송정보 갱신(2026-09-11, 기기군 단위) — 발송 라인(수리반환·교체)의 발송방법·송장·발송일 일괄 정정, 종결 접수도 허용 |
| POST | `/api/as-receipts/[id]/intake` | 입고처리(2026-09-11) — `{ serials[] \| serialsText, receivedAt?, checkedAt? }` 실물 시리얼 대조(정상입고/미입고/미식별입고), 헤더 입고일·확인일·상태 '입고' 자동. USER 이상 — 2026-09-17: 정상입고 전환 라인(결과 NULL·교체)은 기기 INTAKE(리프레시센터, ref별 1회), **종결(완료·취소) 접수는 입력 전부가 '교체' 라인(대기·정상입고)과 일치할 때만 사후 입고 허용**(불일치 400·미식별 라인 생성 없음, 헤더 불변·비고만, 전환 0건은 경고 '변경 사항 없음') |
| POST | `/api/as-receipts/[id]/correct-serial` | 라인 시리얼 보정(2026-09-15) — `{ itemId, serial }` 인입 오타를 미종결 라인 단위로 수정: 원 시리얼 `receiptSerialNo` 보존(최초 1회), 새 시리얼 원장 재매칭·AS 표시, 이전 시리얼의 AS 표시(이 접수 참조) 해제, 비고 이력. 접수 수정 권한 |
| POST | `/api/as-receipts/[id]/intake-confirm` | 입고 대조 접수자 확인(2026-09-11) — `{ type: REMAP\|MARK_RECEIVED\|NOT_RECEIVED\|ACCEPT_EXTRA\|DISCARD_EXTRA, itemId, extraItemId?, comment?, deviceKind? }`. USER 이상 |
| POST | `/api/as-receipts/[id]/registry-confirm` | 원장 정합 확정(2026-09-11) — `{ itemId, modelInput?, productType?, wardName? }` 미등록 신규 등록 / 회수·미배치 재등록 / 타병원 이관 → 접수 병원 배치·라인 연결·AS 표시. USER 이상 |
| POST | `/api/as-receipts/[id]/reopen` | 리오픈(2026-09-11) — 완료·취소 접수를 비종결 상태로(`{ reason, statusId? }` 사유 필수·비고 이력, 기본 대상: 전 라인 종결이면 '발송완료', 정상입고 라인 있으면 '입고', 아니면 '접수'). 헤더만 되돌림(라인·기기현황 불변), 티켓 어댑터 동기화로 재오픈. USER 이상 |
| POST | `/api/as-receipts/[id]/complete` | 최종 완료(2026-09-11, 4. 기기등록 [완료]) — 미종결 라인 0 필수(409) → '완료'·완료일·티켓 CLOSED·비고 이력. USER 이상 |
| POST | `/api/as-receipts/[id]/repair-done` | 라인 수리완료 체크/해제(2026-09-17 — `device_condition_location_design.md` §7.1·§7.2) — `{ itemId, repaired }` 입고된 라인만(400 '입고된 라인만 수리완료 처리할 수 있습니다'), 분실·취소·미회수 400, 결과 확정 라인(선교체)·**종결 접수도 허용**(A-2 — 다른 라인 API의 409 규약과 다름). 라인 `repaired_at/by`(오늘 KST) + 기기 수리완료(REPAIR_DONE ref AS) / 해제는 CORRECT(AS접수), 비고 `[수리완료 …] 시리얼`; 미등록·사용중·폐기 기기는 라인만 기록 + 경고, 재체크 멱등(경고 '변경 사항 없음'), 낙관 가드 409 '동시에 변경되어 다시 시도하세요'는 전파(tx 롤백). VIEWER 제외, `syncTicketClocksSafe`(알림 없음) |
| POST | `/api/as-receipts/[id]/scrap-line` | 라인 기기 폐기(2026-09-17, A-5 — AS 업무 권한과 동일 VIEWER 제외) — `{ itemId, memo(필수, 없으면 400) }` 입고된 라인·원장 연결 라인만(미등록 400), 배치 ACTIVE 409 '배치 중 기기는 먼저 회수하세요', 분실·이미 폐기 400 → 폐기·위치 없음(SCRAP ref AS) + 라인 `repaired_at/by` NULL + 비고 `[폐기 …] memo`. 종결 접수 허용 |

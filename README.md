...
- **계약 수량 = 딜 모델별 수량 1순위(2026-09-02 B-25)**: 기대(도입) 수량은 `sales_deal_devices`(딜×모델, 딜 폼 '기기별 도입 수량')가 1순위 — 행 없는 딜만 `daewoong_device_count` 폴백(ECG에만 기여, 계약별 표에 ⓘ '모델별 수량 미입력'). SpO2는 실측 행이 있으면 hard 대조('ECG 동수 가정' soft는 폴백 전용 병원만), 링BP는 BP 행이 있을 때만 hard. 요약 `deals[].expectedByModel/expectedSource/activeByModel`·커버리지 ECG 계약 툴팁도 동일 산식. sales_* 읽기 전용

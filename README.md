...
├── sales/                            # 영업현황 (ADMIN+SEERS) — dashboard/(대시보드 A·실적, 메인) + deals/(도입현황 — 엑셀 B~AK 표·등록 모달, [id] 딜 상세 편집) + page.tsx(→ dashboard 리다이렉트) + _components/SalesConceptTabs(탭 2개)
...
- **엑셀 다운로드** (2026-08-04, 로그인 사용자 전체): `GET /api/hospitals/export` — 목록 화면과 **동일한 필터(`search`/`sido`/`status`/`type`)를 그대로 받아 페이지네이션 없이 조건에 맞는 전체**를 xlsx로 생성. 12컬럼(병원코드·병원명·HIRA 병원명·종별·상태·시도·시군구·주소·도입형태·도입 병상수·계약일·등록일), 정렬은 목록과 동일(등록 최신순). 병원 테이블에 HIRA 전수(약 8만건)가 들어 있어 무필터 내보내기는 45MB·약 4초가 걸리므로 **1만 건 초과 시 브라우저 확인 대화상자**로 한 번 막는다(하드 상한은 두지 않음 — 종별 '의원' 등 정당한 필터도 3만건대)
...
### 영업현황 (`/sales/dashboard`·`/sales/deals`, 영업/CRM v4 — ADMIN 이상 + SEERS 전용)
- nav '영업 현황(개발중)' → **메인 화면은 대시보드(`/sales/dashboard`, 구 대시보드 A)**, 탭 2개 구성: `대시보드` | `도입현황`. `/sales`는 대시보드로 리다이렉트(구 북마크 호환)
...
- **운행일지 인쇄 (2026-07-21)**: 운행일지 탭 '인쇄' 버튼 → `/vehicle-reservations/logs/print` (현재 차량·기간 필터 유지, 네비 없는 전체 화면) — **A4 가로, 차량별 1장씩 페이지 나눔**. 표준 운행일지 양식(번호·운행일자·운행시간·운전자·운행목적·행선지·계기판거리·주행거리·비고 + 합계), 전체 차량 선택 시 차량마다 별도 시트로 일괄 인쇄

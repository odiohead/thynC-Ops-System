...
- 마스터 시드: `scripts/seed-as-masters.sql` (idempotent — AS_STATUS 7종+매핑('처리중' 삭제 포함)·자동생성 규칙·nav 2행). 스모크: `scripts/as-receipt-smoke.mts` (43항목)
...
- 상태 단계형 7종(접수·수거중·입고·발송·완료·보류·취소 — 선교체 20% 실측 대응, 순서 강제 없음. '처리중'은 2026-09-10 제거 — 수거중·입고·발송이 모두 IN_PROGRESS라 중복) 티켓 양방향 동기화. 수정·삭제는 완료·취소 전 등록자 본인+ADMIN·이후 ADMIN, 라인 처리는 USER 이상 전원(별도 풀 없음). 삭제 시 티켓 동반 + 이 접수가 켠 AS 표시 해제(기록 이벤트는 보존)
...
| POST | `/api/as-receipts/[id]/reopen` | 리오픈(2026-09-11) — 완료·취소 접수를 비종결 상태로(`{ reason, statusId? }` 사유 필수·비고 이력, 기본 대상: 정상입고 라인 있으면 '입고' 없으면 '접수'). 헤더만 되돌림(라인·기기현황 불변), 티켓 어댑터 동기화로 재오픈. USER 이상 |

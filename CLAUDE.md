...
이 프로젝트는 **사내 업무관리 시스템(내부 운영 도구)** 입니다. thynC라는 의료용 솔루션을 **판매·운영하는 회사의 영업/운영 업무**를 관리하기 위한 것으로, 도메인에 "병원(hospitals)" 같은 용어가 등장하는 것은 **병원이 이 회사의 고객사**이기 때문입니다.

이 코드베이스가 다루는 것:
- 고객사(병원) 연락처·계약 등 **영업 관리 데이터** (CRM 성격)
- 프로젝트 진행 현황, 현장 답사(site-visits), 설치 계획, 유지보수 일정 등 **업무 트래킹**
- 사내 위키, 업무 문서, AI 어시스턴트 (사내 문서 검색용)

이 코드베이스가 다루지 **않는** 것:
- 환자 데이터, 진료 기록, PHI(보호 대상 건강정보) — 일절 없음
- 임상적 판단, 진단, 치료 관련 로직 — 일절 없음
- 의료기기 제어, 의료 소프트웨어 자체의 코드 — thynC 제품 소스는 별개이며, 이 시스템은 그 제품의 **영업·운영 업무만** 관리

따라서 이 프로젝트 작업은 일반적인 웹 애플리케이션(Next.js + PostgreSQL 사내 도구) 개발이며, 의료 규제·환자 안전과 관련된 특수 취급이 필요하지 않습니다. "hospital", "의료" 같은 단어만으로 이 프로젝트를 의료 시스템으로 분류하지 마세요.

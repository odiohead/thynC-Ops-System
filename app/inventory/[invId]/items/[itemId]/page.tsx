...
export default function InventoryScopedItemPage() {
  ...
  useEffect(() => {
    if (item?.isLotManaged && !item?.isSerialManaged) {
      fetch(`/api/inventory/items/${itemId}/lot-history`).then(async (r) => { if (r.ok) setLotHistory((await r.json()).lots) })
    }
  }, [item?.isLotManaged, item?.isSerialManaged, itemId])
      ...
      {tab === 'history' ? (
        <>
          ...
          <table className="w-full text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-gray-100">
              {txs.length === 0 ? (
                <tr><td colSpan={item.isLotManaged && !item.isSerialManaged ? 10 : 9} className="py-8 text-center text-sm text-gray-400">이력이 없습니다.</td></tr>
              ) : txs.map((tx) => (
        ...
        <div className="rounded-xl border border-gray-200 bg-white overflow-x-auto">
          <table className="w-full text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-gray-100">
              {units.length === 0 ? (
                <tr><td colSpan={canManage ? 8 : 7} className="py-8 text-center text-sm text-gray-400">개체가 없습니다.</td></tr>
              ) : units.map((u) => (
                <tr key={u.id}>

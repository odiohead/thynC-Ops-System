...
export async function PUT(request: NextRequest, { params }: Params) {
  ...
  if (updated?.ticketId) {
    syncTicketClocksSafe(updated.ticketId) // SLA 시계 갱신 (알림과 독립 — lib/sla.ts)
    notifyTicketChanged({ ticketId: updated.ticketId, actorName: authUser.name, actorId: authUser.userId }).catch(() => {})
  }
...
export async function DELETE(request: NextRequest, { params }: Params) {
  ...
  if (!isAdminOrAbove(authUser.role) && !(isUserOrAbove(authUser.role) && (await hasPermission(authUser, 'install_plan.admin'))))

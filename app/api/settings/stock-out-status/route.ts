...
// 출고요청 워크플로 상태 (STOCK_OUT_STATUS) — stock_out_request_design.md §5, 어댑터 SOP 2단계
export async function GET() {
  const statusCodes = await prisma.statusCode.findMany({
    where: { category: 'STOCK_OUT_STATUS' },
    orderBy: { order: 'asc' },
  })
...
export async function POST(request: NextRequest) {
  ...
  const existing = await prisma.statusCode.findFirst({ where: { name, category: 'STOCK_OUT_STATUS' } })
  if (existing) {
  ...
  const statusCode = await prisma.statusCode.create({
    data: {
      name: name.trim(), order: order ?? 0, color: color ?? null, category: 'STOCK_OUT_STATUS',
      ticketStatus,

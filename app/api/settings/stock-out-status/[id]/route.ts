...
export async function PUT(request: NextRequest, { params }: Params) {
  ...
  if (!name?.trim()) {
    return NextResponse.json({ error: '출고업무 상태명을 입력해주세요.' }, { status: 400 })
  }

  const duplicate = await prisma.statusCode.findFirst({
    where: { name: name.trim(), category: 'STOCK_OUT_STATUS', id: { not: id } },
  })
  if (duplicate) {
    return NextResponse.json({ error: '이미 존재하는 출고업무 상태명입니다.' }, { status: 409 })
  }

  const before = await prisma.statusCode.findUnique({ where: { id } })
  if (!before || before.category !== 'STOCK_OUT_STATUS') {
    return NextResponse.json({ error: '출고업무 상태를 찾을 수 없습니다.' }, { status: 404 })
  }
  ...
  await logAudit({
    ...
    action: 'UPDATE',
    resource: 'setting:stock_out_status',
    resourceId: id,
...
export async function DELETE(request: NextRequest, { params }: Params) {
  ...
  if (!sc || sc.category !== 'STOCK_OUT_STATUS') {
    return NextResponse.json({ error: '출고업무 상태를 찾을 수 없습니다.' }, { status: 404 })
  }

  const inUse = await prisma.stockOutRequest.count({ where: { statusId: id } })
  if (inUse > 0) {
    return NextResponse.json({ error: `이 상태를 사용하는 출고요청이 ${inUse}건 있어 삭제할 수 없습니다.` }, { status: 409 })
  }

...
export async function PUT(request: NextRequest, { params }: Params) {
  ...
  if (!name?.trim()) {
    return NextResponse.json({ error: '장애��형명을 입력해주세요.' }, { status: 400 })
  }

  const duplicate = await prisma.statusCode.findFirst({
    where: { name: name.trim(), category: 'MAINTENANCE_TYPE', id: { not: id } },
  })
  if (duplicate) {
  ...
  await logAudit({
    ...
    action: 'UPDATE',
    resource: 'setting:maintenance_type',
    resourceId: id,
...
export async function DELETE(request: NextRequest, { params }: Params) {
  ...
  const id = parseInt(params.id)
  if (isNaN(id)) return NextResponse.json({ error: '잘���된 ID입니다.' }, { status: 400 })

  const sc = await prisma.statusCode.findUnique({ where: { id } })
  if (!sc || sc.category !== 'MAINTENANCE_TYPE') {
    return NextResponse.json({ error: '장애유형을 찾을 수 없습니���.' }, { status: 404 })
  }

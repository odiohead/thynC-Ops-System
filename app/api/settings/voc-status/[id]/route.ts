...
export async function PUT(request: NextRequest, { params }: Params) {
  ...
  const duplicate = await prisma.statusCode.findFirst({
    where: { name: name.trim(), category: 'VOC_STATUS', id: { not: id } },
  })
  if (duplicate) {
  ...
  if (!before || before.category !== 'VOC_STATUS') {
    return NextResponse.json({ error: 'VOC 상태를 찾을 수 없습니다.' }, { status: 404 })
  }
...
export async function DELETE(request: NextRequest, { params }: Params) {
  ...
  if (!sc || sc.category !== 'VOC_STATUS') {
    return NextResponse.json({ error: 'VOC 상태를 찾을 수 없습니다.' }, { status: 404 })
  }

  const inUse = await prisma.vocReceipt.count({ where: { statusId: id } })
  if (inUse > 0) {
    return NextResponse.json({ error: `이 상태를 사용하는 VOC 접수가 ${inUse}건 있어 삭제할 수 없습니다.` }, { status: 409 })
  }

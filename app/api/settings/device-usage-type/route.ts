...
import { DEVICE_USAGE_TYPE_CATEGORY, USAGE_TYPE_VALUES } from '@/lib/deviceRegistryShared'

export const dynamic = 'force-dynamic'

/**
 * 기기 용도 마스터 (StatusCode DEVICE_USAGE_TYPE — 2026-09-01 결정: 판매용 SALE / 평가용 EVAL, 유닛 속성 §5c)
 * value는 시스템 의미(EVAL은 계약 대조 제외)로 로직이 결합돼 있어 UI에서는 라벨(name)·순서만 편집.
 * GET 로그인 / POST·PUT·DELETE ADMIN+ (§8.1 마스터는 isAdminOrAbove 직접 판정) — device-recovery-reason과 같은 패턴
 */
...
export async function POST(request: NextRequest) {
  ...
  if (value != null && value !== '') {
    if (typeof value !== 'string' || !(USAGE_TYPE_VALUES as readonly string[]).includes(value.trim())) {
      return NextResponse.json({ error: `시스템 값은 ${USAGE_TYPE_VALUES.join('·')} 중 하나여야 합니다.` }, { status: 400 })
    }
    sysValue = value.trim()
    const dupValue = await prisma.statusCode.findFirst({ where: { category: DEVICE_USAGE_TYPE_CATEGORY, value: sysValue } })
    if (dupValue) return NextResponse.json({ error: `시스템 값 ${sysValue}은(는) 이미 '${dupValue.name}'에 지정되어 있습니다.` }, { status: 409 })
  }

  const existing = await prisma.statusCode.findFirst({ where: { name: trimmed, category: DEVICE_USAGE_TYPE_CATEGORY } })
  if (existing) return NextResponse.json({ error: '이미 존재하는 용도명입니다.' }, { status: 409 })

  const statusCode = await prisma.statusCode.create({
    data: { name: trimmed, order: orderNum, category: DEVICE_USAGE_TYPE_CATEGORY, value: sysValue },
  })

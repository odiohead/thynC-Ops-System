...
export default function DaewoongSelectModal({
  isOpen,
  onClose,
  onSelect,
  currentAssigneeIds,
  ...
  const [data, setData] = useState<DaewoongUser[]>([])
  ...
  const fetchData = useCallback(async () => {
    ...
    if (res.ok) {
      ...
      const users: DaewoongUser[] = json.data ?? []
      setData(users)
      setTotal(json.total ?? 0)

      // Update names for checked items
      setCheckedMap((prev) => {
  ...
  function toggleCheck(user: DaewoongUser) {
    setCheckedMap((prev) => {
      const next = new Map(prev)
      if (next.has(user.id)) {
        next.delete(user.id)

...
interface QueueItem {
  ...
  fileUrl: string
  status: string
  installPlanId: number | null
  installPlan: { id: number; planCode: string | null } | null
  createdAt: string
}
...
export default function MailQueuePage() {
  ...
  async function handleSync() {
    ...
    setSyncMessage('')
    try {
      const res = await fetch('/api/mail-queue/sync', { method: 'POST' })
      const data = await res.json()
      if (data.newCount > 0) {
  ...
  async function handleRegister() {
    ...
    setRegistering(true)
    try {
      const res = await fetch(`/api/mail-queue/${registerTarget.id}`, {
        method: 'PUT',
  ...
  async function handleIgnore(id: number) {
    ...
    await fetch(`/api/mail-queue/${id}`, { method: 'DELETE' })
    router.refresh()
    fetchItems()
  }
  ...
  async function handleBulkDelete() {
    ...
    await fetch(`/api/mail-queue?status=${activeTab}`, { method: 'DELETE' })
    router.refresh()
    fetchItems()
  }
      ...
      <div className="mx-auto max-w-full px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          ...
          <div className="mt-2 flex items-center justify-between">
            <h1 className="text-2xl font-bold text-gray-900">설치계획 요청 메일 확인</h1>
                  ...
                  filtered.map((item) => (
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap px-3 py-3">
                        ...
                        {item.status === 'registered' && item.installPlan && (
                          <Link
                            href={`/install-plans/${item.installPlan.id}`}
                            className="text-xs font-medium text-blue-600 hover:text-blue-800"
                          >
                            → {item.installPlan.planCode}
                          </Link>
                        )}
                      </td>
                    </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
              <h2 className="text-base font-semibold text-gray-900">설치계획(가안) 등록</h2>
              <button

...
export default function MaintenanceStatusSettingsPage() {
  ...
  async function handleSaveEdit(sc: StatusCode) {
    ...
    setBusy(true)
    const res = await fetch(`/api/settings/maintenance-status/${sc.id}`, {
      method: 'PUT',
  ...
  async function handleDelete(sc: StatusCode) {
    if (!confirm(`'${sc.name}' 유지보수 상태를 삭제하시겠습니���?`)) return
    setBusy(true)
    const res = await fetch(`/api/settings/maintenance-status/${sc.id}`, { method: 'DELETE' })
    if (res.ok) {
      router.refresh()
      await fetchStatuses()
    } else {
  ...
  async function handleMove(index: number, direction: 'up' | 'down') {
    ...
    await Promise.all([
      fetch(`/api/settings/maintenance-status/${current.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: current.name, order: target.order, color: current.color }),
      }),
      fetch(`/api/settings/maintenance-status/${target.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">유지보수 상태 관리</h1>
            <p className="mt-1 text-sm text-gray-500">유지보수 진행 상태와 표시 색상을 관리합니다.</p>
          </div>
          {!isAdding && (
            <button
              type="button"
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan={4} className="py-12 text-center text-sm text-gray-400">등록된 유지보수 상태가 없습니다.</td>
                </tr>
              ) : (

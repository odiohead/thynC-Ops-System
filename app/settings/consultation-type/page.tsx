...
export default function ConsultationTypeSettingsPage() {
  ...
  async function fetchStatuses() {
    const res = await fetch('/api/settings/consultation-type')
  ...
  async function handleSaveEdit(sc: StatusCode) {
    ...
    setBusy(true)
    const res = await fetch(`/api/settings/consultation-type/${sc.id}`, {
      method: 'PUT',
  ...
  async function handleDelete(sc: StatusCode) {
    if (!confirm(`'${sc.name}' 상담유형을 삭제하시겠습니까?`)) return
    setBusy(true)
    const res = await fetch(`/api/settings/consultation-type/${sc.id}`, { method: 'DELETE' })
    if (res.ok) {
      router.refresh()
      await fetchStatuses()
    } else {
  ...
  async function handleMove(index: number, direction: 'up' | 'down') {
    ...
    await Promise.all([
      fetch(`/api/settings/consultation-type/${current.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: current.name, order: target.order, color: current.color }),
      }),
      fetch(`/api/settings/consultation-type/${target.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">상담유형 관리</h1>
            <p className="mt-1 text-sm text-gray-500">AI 어시스턴트 상담에 적용되는 상담유형과 표시 색상을 관리합니다.</p>
          </div>
          {!isAdding && (
            <button
              type="button"
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">상담유형명</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan={4} className="py-12 text-center text-sm text-gray-400">등록된 상담유형이 없습니다.</td>
                </tr>
              ) : (

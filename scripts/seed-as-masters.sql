...
-- '처리중' 제거 (2026-09-10) — 사용 중인 접수가 없을 때만 (FK SET NULL 방지)
DELETE FROM status_codes sc WHERE sc.category='AS_STATUS' AND sc.name='처리중'
  AND NOT EXISTS (SELECT 1 FROM as_receipts r WHERE r.status_id = sc.id);
